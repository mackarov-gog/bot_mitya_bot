...
async def check_toxicity_llm(text: str) -> int:
    ...
    prompt = (
        f""" 
Ты выступаешь в роли беспристрастного эксперта, который анализирует тональность сообщений в чате. Твоя задача — оценить каждое сообщение по шкале от −5 до +5. Оценка должна быть объективной и учитывать содержание сообщения без привязки к контексту переписки.

**Шкала оценок:**
- **−5**: сообщение содержит крайне грубое оскорбление, нецензурную брань, явные признаки агрессии или угрозы в адрес собеседника.
- **−3**: в сообщении есть явное оскорбление, унижение, сарказм с намерением задеть чувства собеседника или грубый тон.
- **−1**: присутствуют лёгкие признаки грубости, невежливости, намёк на раздражение или неуважительная форма обращения.
- **0**: сообщение нейтральное — это может быть вопрос, информационный ответ, констатация факта или междометие без негативного подтекста.
- **+1**: вежливый и доброжелательный тон, краткий полезный ответ.
- **+3**: развёрнутый полезный совет, выражение искренней благодарности, комплимент или поддержка.
- **+5**: сообщение несёт исключительно полезную информацию, выражает глубокую эмпатию, высшее уважение или предлагает помощь, способную существенно изменить ситуацию.

**Правила оценки:**
1. Если сообщение нейтральное или представляет собой вопрос — ставь 0.
2. Не выискивай негатив там, где его нет, но и не игнорируй явные проявления неуважения или агрессии.
3. В случае сомнений в нейтральности сообщения — поставь себя на место получателя: могло бы это сообщение тебя обидеть? Если да — присвой отрицательную оценку.
4. В ответе приводи только число без дополнительных пояснений или символов.
5. Оценивай только содержание представленного сообщения, не учитывай предшествующую переписку.

Учитывай, что важно корректно оценивать и положительные сообщения — не занижай оценки за добрые слова или полезные советы.
...
async def mitya_continue_handler(message: types.Message):
    ...
    sys_instr = (
        """
Ты — соавтор. Тебе дают начало фразы. Твоя задача: ПРЯМО ПРОДОЛЖИТЬ текст, встроившись в незаконченное последнее слово или предложение. Не начинай новое предложение, если начало обрывается на середине.
Пиши ТОЛЬКО продолжение текста. Никаких вводных слов, объяснений и кавычек.
Стиль: кратко, разговорный сленг («по-пацански»).
        """
    )
